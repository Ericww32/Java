...
public class XmlUtility
{
			...
			throws NumberFormatException
	{
		...
		return new LoginResponse(loggedIn, sessionId);
	}
	...
	private static Document generateLoginResponseXml(LoginResponse response)
	{
		...
		Attribute version = new Attribute("version", "1.0");
		root.setAttribute(version);
		//String pinFmt = String.format("%d", response.getLoggedIn());
		Attribute loggedIn = new Attribute("logged-in", response.getLoggedIn().toString());
		root.setAttribute(loggedIn);
		String sessionIdFmt = String.format("%d", response.getSessionId());
		Attribute sessionId = new Attribute("session-id", sessionIdFmt);
		root.setAttribute(sessionId);
